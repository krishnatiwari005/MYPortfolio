...
import { Globe, GitBranch, Link2, MessageCircle, Mail, Code, Plus, Trash2 } from 'lucide-react';
import { useEffect, useState } from 'react';

export const HeroManager = () => {
  const queryClient = useQueryClient();

  // Multi-tagline state (stored pipe-separated in DB)
  ...
  useEffect(() => {
    if (hero) {
      reset(hero);
      // Parse pipe-separated taglines from DB
      const parsed = hero.tagline
        ? hero.tagline.split('|').map((t: string) => t.trim()).filter(Boolean)
        : [''];
      setTaglines(parsed.length > 0 ? parsed : ['']);
    }
  }, [hero, reset]);
  ...
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<Hero>) => {
      // Merge taglines into pipe-separated string
      const combinedTagline = taglines.filter((t) => t.trim()).join(' | ');

      const { error: upsertError } = await supabase.from('hero').upsert({
        id: true,
        ...formData,
        tagline: combinedTagline,
  ...
  return (
    <form id="admin-active-form" onSubmit={handleSubmit((data) => saveMutation.mutate(data))} className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {/* Multi-Tagline Manager */}
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <label className="text-xs font-semibold text-text-secondary">Rotating Descriptions</label>
              <p className="text-[10px] text-text-tertiary mt-0.5">They cycle every 3.5s on your portfolio</p>
            </div>
            <button
              type="button"
              onClick={() => setTaglines((prev) => [...prev, ''])}
              className="flex items-center gap-1 text-[10px] font-bold text-accent-primary border border-accent-primary/40 px-2.5 py-1 rounded-lg hover:bg-accent-primary/10 transition-all"
            >
          ...
          <div className="space-y-2">
            {taglines.map((line, idx) => (
              <div key={idx} className="flex items-center gap-2">
                <span className="text-[10px] font-mono text-text-tertiary w-5 shrink-0">#{idx + 1}</span>
                <input
                  type="text"
                  value={line}
                  onChange={(e) => {
                    const updated = [...taglines];
                    updated[idx] = e.target.value;
                    setTaglines(updated);
                  }}
                  placeholder={`Description ${idx + 1} (emoji optional)`}
                  className="flex-1 bg-[#001a33] border border-border-default rounded-lg px-3 py-2 text-xs text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-accent-primary transition-colors"
                />
                {taglines.length > 1 && (
                  <button
                    type="button"
                    onClick={() => setTaglines((prev) => prev.filter((_, i) => i !== idx))}
                    className="p-1.5 text-text-tertiary hover:text-error hover:bg-error/10 rounded-lg transition-all shrink-0"
                  >
